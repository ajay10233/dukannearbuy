...
export default function RolesCard() {
    ...
    // const visitProfile = (role) => {
    //     router.push('/getstarted/signup', {
    //         state: {role}
    //     })
    // }
  return (
    <motion.div className="flex flex-col justify-around w-full h-3/4 p-3">
            {
                profiles.map((detail, i) => {
                    return (
                            <Link href={`/getstarted/signup?role=${detail.role}`} className="cursor-pointer flex bg-[var(--secondary-color)] w-full rounded-lg shadow-md p-5" key={i}>
                                ...
                                <div className="flex flex-col justify-center w-2/3">
                                    <h1 className='text-2xl font-bold text-[var(--specialtext)] capitalize'>{detail.name}</h1>
