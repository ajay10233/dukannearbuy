...
export default function Bill() {
  ...
  const filteredBills = bills.filter((bill) => {
    const valid =
      (bill.favorited || !bill.expiry || new Date(bill.expiry) > new Date()) &&
      (!dateFilter || bill.date === dateFilter) &&
      (!typeFilter || bill.type === typeFilter) &&
      (favoriteFilter === null || bill.favorited === favoriteFilter);

    const query = search.toLowerCase();
    const matchesSearch = bill.institution.toLowerCase().includes(query);
